...
class Net(torch.nn.Module):
    ...
    def __init__(self,n_input,n_hidden,n_output):
        ...
        self._control=ControlNet(7,24,3)
...
def g_(data,u):
    ...
    for i in range(len(x)):
        ...
        z[i,:]=torch.ones(3)*sigma*((1+t[i]))*20
    return z
...
xy_data = torch.Tensor(N,6).uniform_(-30,30)
...
# for k in range(1):
    ...
    while i<max_iters:
        ...
        g = g_(data,u)
        t,x,y=data[:,0:1],data[:,1:args.D_in+1],data[:,args.D_in+1:2*args.D_in+1]
        t_x = data[:,0:args.D_in+1].clone().detach().requires_grad_(True)
        # x,y= data[:,0:args.D_in],data[:,args.D_in:2*args.D_in]
        output = torch.mm(torch.tanh(torch.mm(t_x,W1.T)+B1),W2.T)+B2
        ...
        for r in range(args.batch_size):
            ...
            L_V=torch.sum(2*l*x[r,:]*f[r,:])+torch.sum(Vx[0,D_in*r+1:D_in*r+D_in]*f[r,:])+0.5*torch.mm(
                g[r,:].unsqueeze(0),torch.mm(Vxx[D_in*r+1:D_in*r+D_in,D_in*r+1:D_in*r+D_in],g[r,:].unsqueeze(1)))+0.5*torch.sum(
                2*l*g[r,:]**2)
            V_t = Vx[0,D_in*r]
...
# print(model.k)
torch.save(model._control.state_dict(),'ES_tt.pkl')